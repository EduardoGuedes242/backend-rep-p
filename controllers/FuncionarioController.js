...
module.exports = class FuncionarioController{
  ...
  static async cadastrar(req,res){
    ...
    const {nome,pis,email,senha,ativo,celular} = req.body
      ...
      const pisExiste = await Funcionario.findOne({where:{pis:pis,EmpresaId:empresaid}})
      if(pisExiste){
        res.status(422).json({message:'já existe um funcionário com esse PIS',pis})
        return
      }

      const cpfExiste = await Funcionario.findOne({where:{cpf:cpf,EmpresaId:empresaid}})
      if(cpfExiste){
        res.status(422).json({message:'Já existe um funcionário com esse CPF',cpf})
        return
      }
      ...
      if(emailExiste){
        ...
        res.status(422).json({message:'Este e-mail já está sendo utilizado',emailExiste})
        return
      }

      if(celular != null) { 
      const celularExiste = await Funcionario.findOne({where:{celular:celular}})
      if(celularExiste){
        celularExiste.senha = undefined
        res.status(422).json({message:'Este celular já está sendo utilizado',celularExiste})
        return
      }
    }
      var passwordHash = ''
      ...
      try{
          ...
          {nome:nome,pis:pis,cpf:cpf,email:email,senha:passwordHash,EmpresaId:empresaid,celular:celular,ativo:true})//empresaid:EmpresaId,
  ...
  static async showAllFuncionarios(req,res){
    ...
    try{
      const funcionario = await Funcionario.findAll({ attributes: { exclude: ['senha'] }, where: { EmpresaId: empresa.id } });
      res.status(200).json(funcionario)
    }catch(err){
      res.status(500).json(err.message)
    }
  ...
  static async funDel(req,res){
    ...
    const funcionario = await Funcionario.findOne({where:{cpf:cpfPai,EmpresaId:empresa.id}})
    ...
    if (marcacoes){
        return res.status(422).json({message:'Este funcionario já possui marcações e não pode ser excluido'})
    }
  ...
  static async editFun(req,res){
    ...
    if (pis && pis !== funcionario.pis){
        const pisExiste = await Funcionario.findOne({where:{pis:pis,EmpresaId:empresa.id}})
        if(pisExiste){
            return res.status(422).json({message:'já existe um funcionário com esse PIS',pis})
        }
    }
    funcionario.pis = pis

    if (cpf && cpf !== funcionario.cpf){
        const cpfExiste = await Funcionario.findOne({where:{cpf:cpf,EmpresaId:empresa.id}})
        if(cpfExiste){
            return res.status(422).json({message:'Já existe um funcionário com esse CPF',cpf})
        }
    }
    ...
    if (email && email !== funcionario.email){
        ...
        if(emailExiste){
            emailExiste.senha = undefined
            return res.status(422).json({message:'Este e-mail já está sendo utilizado',emailExiste})
        }
    }
    ...
    if (celular && celular !== funcionario.celular){
      ...
      if(celularExiste){
        celularExiste.senha = undefined
          return res.status(422).json({message:'Este celular já está sendo utilizado',emailExiste})
      }
  }
  ...
  funcionario.celular = celular
    ...
    try{
        ...
        const funcRep = await FunRep.findAll({where:{FuncionarioId:funcionario.id}})
        ...
        for (const key in funcRep) {
          ...
          let codigoCrc = ultimaMarc.nsr + tipoRegistro + date + cpfResponsavel + 'A' + funcionario.cpf + funcionario.nome
