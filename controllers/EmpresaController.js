...
module.exports = class EmpresaController{
  ...
  static async cadastrar(req,res){
    ...
    //if(emp.id != 1){return res.status(401).json({message:'Acesso Negado boy!'})}
    ...
    const {contrato,login,senha,razao,fantasia,email} = req.body
      ...
      const contratoExiste = await Empresa.findOne({where:{contrato:contrato}})
      if(contratoExiste){
      ...
      const passwordHash = await bcrypt.hash(senha, salt)
        ...
        try{
            const newEmp = await Empresa.create({cnpj:cnpj,contrato:contrato,login:login,senha:passwordHash,razao:razao,fantasia:fantasia,email:email,ativo:true})
            // newEmp.save();
  ...
  static async showAllEmpresa(req,res){
    ...
    try{
      const empresas = await Empresa.findAll({ attributes: { exclude: ['senha'] }});
